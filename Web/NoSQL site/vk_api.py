...
class User:
    ...
    def popushennost_test(self):
        ...
        for zashkvar in popushennost_list:
            if zashkvar in self.groups:
                groups.append(zashkvar)

        if groups:
            user_string.append("О нет! Пользователь попущен")
            user_string.append("Он состоит в следующих пабликах:")
            for public in groups:
                user_string.append(requests.get(HOST + 'groups.getById', params={'group_id': public, 'access_token': access_token, 'v': VERSION}).json()["response"][0]["name"] + " id =" + str(public))
        else:
            user_string.append("Хвала летающему макаронному монстру! Пользователь не попущен!!!")
        return user_string
    ...
    def user_info(self):
        ...
        user_string = [
            ...
            f"Пол: {self.sex}",
            f"Родился {self.birthday}",
            f"Насчитывает {self.wall_count} постов",
            f"Получил {wall_info['likes']} лайков, {wall_info['reposts']} репостов, {wall_info['views']} просмотров",
            f"Скопипастил {wall_info['copypasted']} постов"
        ]
        return user_string
